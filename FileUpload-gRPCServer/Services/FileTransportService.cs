...
namespace FileUpload_gRPCServer.Services
{
    public class FileTransportService : FileService.FileServiceBase
    {
        readonly IWebHostEnvironment _webHostEnvironment; // hosttaki değerlere erişmek için kullanılır
        ...
        {
            // Streamin yapılacağı dizin
            string path = Path.Combine(this._webHostEnvironment.WebRootPath, "Files");
        ...
        public override async Task FileDownload(grpcFileTransportServer.FileInfo request, IServerStreamWriter<BytesContent> responseStream, ServerCallContext context)
        {
            ...
            // response edilmesi gerektiğini belirtmek için using ile işaretledik
